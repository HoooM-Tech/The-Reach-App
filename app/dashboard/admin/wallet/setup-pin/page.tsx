    ...
    if (step === 'retype' && retypePin.length === 4) {
      ...
      try {
        ...
        if (data.success) {
          ...
          router.push('/dashboard/admin/wallet/setup-pin/success');
        } else {
        ...
        <div className="bg-white rounded-3xl p-6 shadow-sm mb-8">
          ...
          <div className="flex gap-3 justify-center mb-8">
            {[0, 1, 2, 3].map((index) => {
              ...
              return (
                <div key={index} className={`w-14 h-14 sm:w-16 sm:h-16 rounded-xl border-2 flex items-center justify-center transition-colors ${isActive ? 'border-[#FF6B35] bg-[#FF6B35]/5' : value ? 'border-gray-300 bg-gray-50' : 'border-gray-200'}`}>
          ...
          <button
            ...
            className={`w-full py-4 rounded-2xl font-semibold transition-colors ${!isSubmitting && ((step === 'enter' && pin.length === 4) || (step === 'retype' && retypePin.length === 4)) ? 'bg-[#1E3A5F] text-white hover:bg-[#1E3A5F]/90' : 'bg-gray-200 text-gray-400 cursor-not-allowed'}`}
          >
