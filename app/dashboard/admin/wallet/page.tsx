...
import { X, ChevronRight, Eye, EyeOff, ArrowUpRight, Bell, SlidersHorizontal, RefreshCw, AlertCircle, TrendingUp } from 'lucide-react';
...
export default function AdminWalletPage() {
  ...
  const fetchWalletData = useCallback(async () => {
    try {
      ...
      const [balanceRes, txRes] = await Promise.all([
        ...
        fetch('/api/wallet/transactions?limit=4', { credentials: 'include' }),
      ]);
      ...
      if (txRes.ok) {
        ...
        // Compute basic revenue stats from transactions
        const commissionTxs = txList.filter((t: Transaction) =>
          (t.type === 'credit' || t.type === 'deposit') && t.category === 'commission'
        );
        const thisMonthTotal = commissionTxs.reduce((sum: number, t: Transaction) => sum + Math.abs(t.amount), 0);
        setRevenueStats({
          thisMonth: thisMonthTotal,
          monthGrowth: 0,
          totalCommission: thisMonthTotal,
          transactionCount: commissionTxs.length,
        });
      }
    } catch (err) {
  ...
  // SETUP PROMPT - wallet not set up yet
    ...
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center px-4">
        <div className="text-center max-w-md w-full">
          ...
          {/* Setup Link Card */}
          <button
            onClick={() => router.push('/dashboard/admin/wallet/setup-pin')}
      ...
      <main className="max-w-2xl mx-auto px-4 sm:px-6 pb-8 space-y-6">
        ...
        <div className="bg-white rounded-2xl p-5 sm:p-6 shadow-sm border border-gray-100">
          ...
          <div className="flex gap-3">
            <button
              onClick={() => router.push('/dashboard/admin/wallet/withdraw')}
              className="flex-1 py-3.5 bg-[#1E3A5F] text-white rounded-full font-medium hover:bg-[#1E3A5F]/90 transition-colors"
            >
              Withdraw
            </button>
            <button
        ...
        {/* Revenue Stats (Admin Specific) */}
        {revenueStats && revenueStats.transactionCount > 0 && (
          <div className="grid grid-cols-2 gap-3 sm:gap-4">
            <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
              <p className="text-xs text-gray-500 mb-1">This Month Revenue</p>
              <h3 className="text-xl sm:text-2xl font-bold text-gray-900 mb-1">
                {formatAmount(revenueStats.thisMonth)}
              </h3>
              {revenueStats.monthGrowth > 0 && (
                <div className="flex items-center gap-1 text-green-500 text-xs">
                  <TrendingUp size={14} />
                  <span>{revenueStats.monthGrowth}%</span>
                </div>
              )}
            </div>
            <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
              ...
              <h3 className="text-xl sm:text-2xl font-bold text-gray-900 mb-1">
                {formatAmount(revenueStats.totalCommission)}
              </h3>
              <p className="text-xs text-gray-500">{revenueStats.transactionCount} transactions</p>
            </div>
          </div>
        )}
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            ...
            {transactions.length > 0 && (
              <button
                onClick={() => router.push('/dashboard/admin/wallet/transactions')}
