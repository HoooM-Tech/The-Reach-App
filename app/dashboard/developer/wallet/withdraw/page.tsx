...
export default function WithdrawPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (banksRes.ok) {
        ...
        const accounts: BankAccount[] = banksJson.data || banksJson.bankAccounts || [];
        // Pick primary or first account, or check sessionStorage for recently selected
  ...
  const handleContinue = () => {
    ...
    // Store withdrawal data in sessionStorage for the review page
  ...
  return (
    <div className="min-h-screen bg-[#FDFBFA]">
      {/* Fixed Header */}
      <header className="fixed top-0 left-0 right-0 bg-[#FDFBFA] px-4 sm:px-6 py-4 flex items-center justify-between z-50">
        <button
          aria-label="Back"
          title="Back"
            ...
            <div className="grid grid-cols-3 gap-2 sm:gap-3">
              {[1, 2, 3, 4, 5, 6, 7, 8, 9].map((num) => (
                <button
                  ...
                  className="aspect-square bg-gradient-to-br from-[#F9FAFB] to-[#F3F4F6] rounded-2xl flex items-center justify-center shadow-sm hover:shadow-md active:scale-95 transition-all"
                >
              ...
              <button
                ...
                className="aspect-square bg-gradient-to-br from-[#111827] to-[#1F2937] rounded-2xl flex items-center justify-center shadow-md hover:shadow-lg active:scale-95 transition-all"
              >
