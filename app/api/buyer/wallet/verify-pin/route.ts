...
/**
 * POST /api/buyer/wallet/verify-pin
 * Verify wallet PIN (buyer only)
 */
  ...
  try {
    ...
    if (wallet.pin_locked_until) {
      ...
      if (lockedUntil > new Date()) {
        ...
        throw new AuthenticationError(`PIN is locked. Try again in ${minutesLeft} minute(s).`);
      }
      await supabase
        .from('wallets')
    ...
    if (!wallet.pin_hash) throw new ValidationError('PIN not set. Please set up your wallet.');
    ...
    if (!isValid) {
      ...
      if (newAttempts >= maxAttempts) {
        ...
        await supabase
          ...
          .eq('id', wallet.id);
        throw new AuthenticationError('Too many failed attempts. PIN locked for 30 minutes.');
      }
      await supabase.from('wallets').update({ pin_attempts: newAttempts }).eq('id', wallet.id);
      throw new AuthenticationError(`Invalid PIN. ${maxAttempts - newAttempts} attempt(s) remaining.`);
    }
